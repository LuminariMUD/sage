...
- **Durable Graphiti Extraction Routing (0.7.20)**
  ...
  - Unified route retry/fallback classification with immutable provider-call failure records so malformed/schema/output failures, quota exhaustion, transient HTTP failures, and rejected requests cannot receive contradictory durable identities.
  ...
  - Verified 87 focused route/provider/worker tests and the complete network-isolated fast gate (317 passed, 6 skipped, 114 intentionally deselected). No provider/model request, benchmark, graph claim, ingestion, migration, live mutation, or worker restart was performed; the operator-requested worker freeze remains active.
