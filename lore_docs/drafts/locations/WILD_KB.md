...
# The Luminari wilderness system creates a massive 2048x2048 coordinate grid (-1024 to +1024 on both axes) using procedural generation with Perlin noise algorithms.

  Key Components:

  1. Terrain Generation: Multiple noise layers (elevation, moisture, temperature) combine to create realistic biomes - from oceans and beaches to forests, mountains, and
  deserts based on elevation/climate thresholds.
  2. Room Management:
    - Static rooms (VNUMs 1000000-1003999): Pre-built permanent locations
    - Dynamic rooms (VNUMs 1004000-1009999): Created on-demand as players explore, recycled when unused
  3. Coordinate System: Origin at (0,0), with North=+Y, South=-Y, East=+X, West=-X. Room coordinates stored in world[room].coords[].
  4. Features:
    - Regions: Polygonal areas that modify terrain (stored in MySQL)
    - Paths: Linear features like roads/rivers
    - Resources: 10 types distributed based on terrain
    - Spatial Audio: 3D positional sound with distance calculations
    - PubSub Events: Real-time wilderness event distribution
  5. Player Experience: Standard movement changes coordinates by ±1, with automatic room creation, 21x21 automap display, and resource discovery/mapping commands.

  The system uses KD-Tree indexing for performance and integrates with weather, combat modifiers, and quest systems.
